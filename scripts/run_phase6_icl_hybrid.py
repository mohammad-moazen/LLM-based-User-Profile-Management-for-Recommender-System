"""Run the final controlled ICL baseline with hybrid output validation.

ICL semantics are preserved: interactions through t-2 are earlier context, the
purchase at t-1 is presented as an in-context demonstrated outcome, and the
current frozen candidates are ranked. Mechanical output policy matches final
PURE, Sequential, and Recency: direct ranking first, one fresh rank-map fallback
only after strict structural failure, and no post-generation repair.
"""
...
from pure_recommender.baselines import build_icl_messages, parse_complete_ranking, target_rank
from pure_recommender.baselines.icl_rankmap import build_icl_rankmap_messages
...
EXPERIMENT = "phase6_icl_hybrid_full_v1"
RUN_VERSION = "icl_hybrid_full_v1"
CONFIG_PATH = REPO_ROOT / "config" / "phase6_icl_hybrid.toml"
...
def _publish(payload: dict[str, object], status_word: str) -> None:
    ok, message = publish_handoff(
        REPO_ROOT,
        payload,
        commit_message=f"handoff: phase6 ICL hybrid {status_word.lower()}",
        auto_push=True,
    )
...
def main() -> int:
    ...
    if len(sessions) != 94:
        raise RuntimeError(f"Final ICL workload must contain 94 frozen sessions; found {len(sessions)}")
    if config.experiment.resume:
        raise RuntimeError("Final ICL rerun must start cleanly with experiment.resume=false")

    client = OpenAICompatibleLLMClient(base_url=llm_config.base_url, timeout_seconds=llm_config.timeout_seconds)
    visible_models = client.list_models()
    if llm_config.model not in visible_models:
        raise RuntimeError(f"Configured model {llm_config.model!r} is not exposed by local server; visible={visible_models}")
    ...
    print("=" * 108)
    print("PHASE 6 — FINAL ICL BASELINE / HYBRID OUTPUT V1")
    print("=" * 108)
    ...
    print("Method inputs             : t-2 earlier history + t-1 demonstrated purchase + candidates")
    print("Primary                   : direct ranking array with structured schema")
    print("Fallback                  : one fresh ICL rank-map request after structural parse failure")
    ...
    for ordinal, session in enumerate(sessions, start=1):
        ...
        try:
            ...
            if len(history) < 2:
                raise ValueError(f"ICL session {session_id!r} requires at least two observed purchases")
            recent_title = str(history[-1]["title"])
            earlier_count = len(history) - 1

            direct_messages = build_icl_messages(history=history, candidate_asins=candidate_asins, item_titles=item_titles)
            ...
            try:
                ...
                fallback_attempts += 1
                fallback_messages = build_icl_rankmap_messages(
                    history=history,
            ...
            row: dict[str, object] = {
                ...
                "earlier_history_length": earlier_count,
                "demonstrated_recent_item_title": recent_title,
                ...
                "session_latency_seconds": elapsed,
            ...
            print(f"[{ordinal:02d}/{len(sessions):02d}] {session_id}: protocol={protocol_used} rank={rank} direct={direct_latency:.2f}s fallback={fallback_latency:.2f}s")
    ...
    summary: dict[str, object] = {
        ...
        "method_inputs": "earlier_purchase_titles_through_t_minus_2_plus_t_minus_1_demonstrated_purchase_and_frozen_candidate_titles",
        "output_protocol": {
            "primary": "direct_complete_numbered_candidate_ranking",
            "fallback_condition": "strict_direct_parser_failure_only",
            "fallback": "fresh_icl_candidate_rank_map_request",
    ...
    _publish({"state": "RESULT", "experiment": EXPERIMENT, "summary": summary, "fallback_rows": compact_fallback_rows, "errors": compact_failures}, status)

    print()
    print("=" * 108)
    print("PHASE 6 FINAL ICL SUMMARY")
    print("=" * 108)
