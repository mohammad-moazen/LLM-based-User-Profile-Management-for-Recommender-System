"""Run the Phase 2 In-Context Learning (ICL) baseline on frozen Phase 1 sessions.

For prediction at time step t, the PURE paper's ICL baseline uses interactions
through t-2 as earlier history and presents the purchased item at t-1 as an
in-context demonstrated recommendation outcome. This runner keeps the same
frozen sessions, candidates, parser, metrics, checkpointing, and local LLM
backend used by the validated Sequential and Recency-Focused baselines.
"""
...
def _validate_frozen_session(
    ...
    if target_index < 2 or target_index >= len(history):
        raise ValueError(
            f"ICL session {session['session_id']!r} requires at least two observed purchases"
        )
    ...
    # Observed history contains interactions through t-1. build_icl_messages()
    # performs the paper-defined split into history through t-2 + recent item t-1.
    return history[:target_index], candidate_asins, target_asin


def main() -> int:
    parser = argparse.ArgumentParser(description="Run PURE Phase 2 ICL baseline")
    parser.add_argument(
        "--config",
        default=str(REPO_ROOT / "config" / "phase2_icl.toml"),
        help="Path to ICL TOML config",
    )
    parser.add_argument(
        "--llm-config",
    ...
    print("\n" + "=" * 88)
    print("PURE PHASE 2 — IN-CONTEXT LEARNING (ICL) BASELINE")
    print("=" * 88)
    ...
    for ordinal, session in enumerate(sessions, start=1):
        ...
        recent_title = str(observed_history[-1]["title"])
        earlier_count = len(observed_history) - 1
        ...
        print(
            ...
            f"earlier_history={earlier_count}, demonstrated_recent={recent_title!r}, "
            f"candidates={len(candidate_asins)} ...",
            flush=True,
        )
