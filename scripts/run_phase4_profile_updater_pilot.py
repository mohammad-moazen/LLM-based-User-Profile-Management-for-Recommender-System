...
The runner selects a deterministic user with enough extracted interactions,
starts from an empty profile, and applies Profile Updater chronologically to the
first configured number of extractions. It publishes a compact handoff for
qualitative review while leaving all Phase 3 artifacts untouched.
"""
...
def _publish(payload: dict[str, object], status_word: str) -> None:
    ok, message = publish_handoff(
        REPO_ROOT,
        payload,
        commit_message=f"handoff: phase4 profile updater pilot {status_word.lower()}",
        auto_push=True,
    )
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Run PURE Phase 4 Profile Updater pilot")
    parser.add_argument(
        "--config",
    ...
    print("=" * 96)
    print("PURE PHASE 4 — PROFILE UPDATER PILOT V1")
    print("=" * 96)
    ...
    print()
    print("=" * 96)
    print("PROFILE UPDATER PILOT SUMMARY")
    print("=" * 96)
