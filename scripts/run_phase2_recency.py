"""Run the Phase 2 Recency-Focused baseline on frozen Phase 1 sessions.

The PURE paper defines Recency-Focused as the Sequential baseline plus an
instruction emphasizing the most recently purchased item at time step t-1.
This runner reuses the frozen histories, candidate sets, parser, evaluation,
checkpointing, and local LLM backend from the validated Sequential pipeline.
"""
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Run PURE Phase 2 Recency-Focused baseline")
    parser.add_argument(
        "--config",
        default=str(REPO_ROOT / "config" / "phase2_recency.toml"),
        help="Path to Recency-Focused TOML config",
    )
    parser.add_argument(
        "--llm-config",
    ...
    print("\n" + "=" * 88)
    print("PURE PHASE 2 — RECENCY-FOCUSED BASELINE")
    print("=" * 88)
    ...
    for ordinal, session in enumerate(sessions, start=1):
        ...
        messages = build_recency_messages(
        ...
        print(
            ...
            f"history={len(observed_history)}, recent={observed_history[-1]['title']!r}, "
        ...
        try:
            ...
            result: dict[str, object] = {
                ...
                "model": llm_config.model,
                "recent_item_title": str(observed_history[-1]["title"]),
                "ranking": ranking,
                "target_rank": rank,
    ...
    print("\n" + "=" * 88)
    print("PHASE 2 RECENCY-FOCUSED SUMMARY")
    print("=" * 88)
